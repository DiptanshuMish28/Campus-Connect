    ...
    def __init__(self, loginId):
        ...
        def view_details():
            ...
            style.configure('Details.TLabel', font=(
                'Helvetica', 14), padding=5)

            # Create labels to show the details
            id_label = ttk.Label(
                details_window, text=f"ID: {values[0]}", style='Details.TLabel')
            id_label.pack()
            name_label = ttk.Label(
                details_window, text=f"NAME: {values[1]}", style='Details.TLabel')
            name_label.pack()
            address_label = ttk.Label(
                details_window, text=f"TITLE: {values[2]}", style='Details.TLabel')
            address_label.pack()
            btype_label = ttk.Label(
                details_window, text=f"SEM: {values[3]}", style='Details.TLabel')
            btype_label.pack()
            atype_label = ttk.Label(
                details_window, text=f"PRICE: {values[4]}", style='Details.TLabel')
            atype_label.pack()
            loc_label = ttk.Label(
                details_window, text=f"DONATION: {values[5]}", style='Details.TLabel')
            loc_label.pack()
            loc_label = ttk.Label(
                details_window, text=f"CONTACT: {values[6]}", style='Details.TLabel')
            loc_label.pack()
            landmark_label = ttk.Label(
                details_window, text=f"CONDITION: {values[7]}", style='Details.TLabel')
            landmark_label.pack()
            up_label = ttk.Label(
                details_window, text=f"DESCRIPTION: {values[8]}", style='Details.TLabel')
            up_label.pack()
